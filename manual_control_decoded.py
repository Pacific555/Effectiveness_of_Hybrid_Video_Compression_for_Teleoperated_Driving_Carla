...
class World(object):
    ...
    def destroy(self):
        ...
        if self.imu_sensor and self.imu_sensor.sensor:
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        '''
        compass = getattr(world.imu_sensor, 'compass', 0.0) if world.imu_sensor else 0.0
...
# ==============================================================================
class BitrateLogger:
    def __init__(self, filename='bitrate_log.csv'):
        self.filename = filename
        self.frame_id = 0
        if not os.path.exists(self.filename):
            with open(self.filename, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Frame', 'Bitrate (kbps)'])

    def log(self, bitrate_kbps):
        with open(self.filename, mode='a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([self.frame_id, round(bitrate_kbps, 2)])
        self.frame_id += 1
# ==============================================================================
# -- CameraManager -------------------------------------------------------------
# ==============================================================================
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self.max_decoder_failures = 3
        self.frame_counter = 0
        self.last_render_time = time.time()
        self.bitrate_logger = BitrateLogger("bitrate_log.csv")
        ...
        try:
            ...
            print(f"[ERROR] Encoder creation failed: {str(e)}")
            raise

        self.hdr_cache = bytearray()
        ...
        self._camera_transforms[1] = (
            carla.Transform(
                carla.Location(x=0.5, y=0.0, z=2.3),
        ...
        with open("processing_times.csv", "w", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["frame_id", "encode_ms", "decode_ms", "total_ms", "avg_ms", "fps"])
    ...
    def setup_dual_cameras(self):
        ...
        seg_bp.set_attribute('image_size_x', str(self.width))
        seg_bp.set_attribute('image_size_y', str(self.height))
    ...
    def _parse_seg_image(self, image):
        ...
        semantic_id_array = np.frombuffer(image.raw_data, dtype=np.uint8).reshape((image.height, image.width, 4))[:, :, 2].copy()
    ...
    def build_demuxer_and_decoder(self):
        ...
        try:
            ...
            codec_id = self.demux.GetNvCodecId()
            print(f"[DBG] Demuxer created, codec_id={codec_id}")
    ...
    def reset_pipeline(self):
        ...
        try:
            ...
            print("[INFO] Encoder reset successfully")
        except Exception as e:
            print(f"[ERROR] Failed to reset encoder: {str(e)}")
        self.demux = None
        self.decoder = None
    ...
    def close(self):
        print("[INFO] Closing CameraManager...")
        if self.out_file and not self.out_file.closed:
            self.out_file.close()
            print("[INFO] Closed output file")
        if self.sensor_rgb and self.sensor_rgb.is_alive:
            self.sensor_rgb.stop()
            self.sensor_rgb.destroy()
            print("[INFO] Destroyed RGB sensor")
        if self.sensor_seg and self.sensor_seg.is_alive:
            self.sensor_seg.stop()
            self.sensor_seg.destroy()
            print("[INFO] Destroyed segmentation sensor")
        print("[INFO] CameraManager closed")

    def render(self, display):
        t0 = time.time()
        frame_id = self.frame_counter
        self.frame_counter += 1
        print(f"[DEBUG] Start render frame {frame_id}, time={t0:.3f}")

        # Calculate FPS
        elapsed = t0 - self.last_render_time
        fps = 1.0 / elapsed if elapsed > 0 else 0
        self.last_render_time = t0
        ...
        try:
            ...
            print(f"[GPU] Free: {free_mb:.2f} MB, Total: {total_mb:.2f} MB")
        except Exception as e:
            print(f"[ERROR] Failed to get GPU memory: {str(e)}")

        # Check CARLA actors
        try:
            world = self._parent.get_world()
            actors = len(world.get_actors())
            print(f"[CARLA] Actors: {actors}")
        except Exception as e:
            print(f"[ERROR] Failed to check CARLA actors: {str(e)}")

        if self.rgb_array is not None and self.seg_array is not None and self.semantic_id_array is not None:
            try:
                # CuPy processing
                ROI_IDS = cp.array([1, 2, 6, 7, 8, 12, 13, 14, 15, 16, 18, 24], dtype=cp.uint8)
                rgb_cp = cp.asarray(self.rgb_array, dtype=cp.float32)
                seg_cp = cp.asarray(self.seg_array, dtype=cp.float32)
                semantic_cp = cp.asarray(self.semantic_id_array, dtype=cp.uint8)
                mask_cp = cp.isin(semantic_cp, ROI_IDS)
                alpha_cp = cp.ones_like(mask_cp, dtype=cp.float32)
                alpha_cp[mask_cp] = 0.0
                alpha_cp = alpha_cp[:, :, None]
                hybrid_cp = rgb_cp * (1 - alpha_cp) + seg_cp * alpha_cp
                hybrid = hybrid_cp.astype(cp.uint8).get()
                ...
                g = hybrid_cp[:, :, 1]
                b = hybrid_cp[:, :, 2]
                a = cp.full_like(r, 255, dtype=cp.uint8)
                argb = cp.stack([b, g, r, a], axis=2).get()
                pkt = self.encoder.Encode(argb)
                pkt2 = self.encoder.EndEncode()
                t_encode_end = time.time()
                encode_time_ms = (t_encode_end - t_encode_start) * 1000

                packet_size = 0
                if pkt:
                    self.out_file.write(pkt)
                    packet_size += len(pkt)
                    print(f"[ENCODE] Packet size={len(pkt)} bytes")
                if pkt2:
                    self.out_file.write(pkt2)
                    packet_size += len(pkt2)
                    print(f"[ENCODE] EndEncode packet size={len(pkt2)} bytes")
                if packet_size == 0:
                    print(f"[ERROR] No packets generated for frame {frame_id}, resetting pipeline...")
                    self.reset_pipeline()
                    return
                ...
                decoded_frame = None
                if self.decoder_failures < self.max_decoder_failures:
                    if not self.got_header:
                        for data in (pkt, pkt2):
                            if data:
                                self.hdr_cache.extend(data)
                        if len(self.hdr_cache) > 1000 or (frame_id > 10 and len(self.hdr_cache) > 0):
                            ...
                            if not nal_starts or all(typ not in [32, 33, 34, 19] for _, typ in nal_starts):
                                print(f"[ERROR] Invalid header data, no VPS/SPS/PPS/IDR found")
                                self.decoder_failures += 1
                                self.reset_pipeline()
                                return
                            end_pos = len(self.hdr_cache)
                            for i, (pos, nal_type) in enumerate(nal_starts):
                                if nal_type == 19 and i >= 3:
                            ...
                            try:
                                ...
                                print(f"[ERROR] Failed to initialize demuxer/decoder: {str(e)}")
                                self.decoder_failures += 1
                                self.reset_pipeline()
                                return
                        elif frame_id > 50:
                            print(f"[ERROR] No header data after {frame_id} frames, resetting pipeline...")
                            self.decoder_failures += 1
                            self.reset_pipeline()
                            return
                    else:
                        ...
                        try:
                            for p in self.demux:
                                ...
                                if surfaces:
                                    buf = np.from_dlpack(surfaces[0])
                                    pitch_px = buf.size // (self.height * 3)
                                    decoded_frame = buf.reshape(self.height, pitch_px, 3)[:, :self.width, :].copy()
                                else:
                                    print(f"[ERROR] No surfaces decoded for frame {frame_id}")
                                    self.decoder_failures += 1
                                break
                        except Exception as e:
                            print(f"[ERROR] Decoder failed: {str(e)}")
                            self.decoder_failures += 1
                            self.reset_pipeline()
                            return
                        ...
                        self.feeder.clear()
                        print(f"[FEED] Buffer cleared at frame {frame_id}")
                ...
                # Pygame display
                raw_surface = pygame.surfarray.make_surface(hybrid.swapaxes(0, 1))
                display.blit(raw_surface, (0, 0))
                font = pygame.font.Font(None, 36)
                raw_label = font.render("Raw", True, (255, 255, 0))
                display.blit(raw_label, (10, 10))
                if decoded_frame is not None:
                    decoded_surface = pygame.surfarray.make_surface(decoded_frame.swapaxes(0, 1))
                    display.blit(decoded_surface, (self.width, 0))
                    decoded_label = font.render("Decoded", True, (255, 255, 0))
                    display.blit(decoded_label, (self.width + 10, 10))
                elif self.decoder_failures >= self.max_decoder_failures:
                    error_label = font.render("Decoder Failed", True, (255, 0, 0))
                    display.blit(error_label, (self.width + 10, 10))
                else:
                    decoding_label = font.render("Decoding...", True, (255, 255, 0))
                    display.blit(decoding_label, (self.width + 10, 10))

                # Log timing
                total_time_ms = (t_decode_end - t0) * 1000
                self.frame_times.append(total_time_ms)
                avg_time_ms = sum(self.frame_times) / len(self.frame_times) if self.frame_times else total_time_ms
                print(f"[TIMING] Frame {frame_id}: Encode={encode_time_ms:.2f}ms, Decode+Display={decode_time_ms:.2f}ms, Total={total_time_ms:.2f}ms, Avg={avg_time_ms:.2f}ms, FPS={fps:.2f}")
                with open("processing_times.csv", "a", newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([frame_id, encode_time_ms, decode_time_ms, total_time_ms, avg_time_ms, fps])
        ...
        if elapsed > self.dt * 2:
            print(f"[ERROR] Frame {frame_id} took too long ({elapsed:.3f}s), resetting pipeline...")
            self.reset_pipeline()
            return

        # Clean up GPU and decoded frame
        if 'decoded_frame' in locals():
            del decoded_frame
        del hybrid_cp, alpha_cp, mask_cp, rgb_cp, seg_cp, semantic_cp
        ...
        print(f"[DEBUG] End render frame {frame_id}, time={time.time():.3f}")
